  ...
  <div>
    <div class="gyl-form-view">
      ...
      <el-form v-show="isShow" ref="form" :model="form" label-width="100px">
        <el-row class="form-content" :class="collapse ? 'auto' : ''">
        ...
        <el-form-item class="gyl-form-btn-wrap pb16">
    ...
    <el-dialog title="设置快捷查询条件" :visible.sync="dialogSetting" width="900px" custom-class="gyl-dialog-setting">
      <h5 class="sub-title">已选字段</h5>
      <div>
        <ul class="form-item-checked">
          <li v-for="(item, index) in checkedList" :key="index">{{ item.title }} <i class="el-icon-clear" @click="removeItem(item)" /></li>
        </ul>
      </div>
      <el-divider />
      <h5 class="sub-title">未选字段</h5>
      <div>
        <ul class="form-item-notChecked">
          <li v-for="(item, index) in notCheckedList" :key="index">{{ item.title }} <i class="el-icon-plus" @click="addItem(item)" /></li>
        </ul>
      </div>
...
export default {
  name: 'Form1',
  components: { CollapseForm, TableSetting },
  data() {
    return {
      settingList: [
        '仓库配置项1',
      ...
      dialogSetting2: false, // 设置快捷查询条件2
      isShow: true,
      collapse: false,
      form: {
        f1: '',
  ...
  methods: {
    ...
    addItem(item) {
      ...
      this.notCheckedList = this.notCheckedList.filter((it) => {
